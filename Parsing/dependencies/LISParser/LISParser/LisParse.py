"""
LisParse.py - Parser for the BAM LIS format (2026)

The LIS format has the following tab-separated column structure:
...
class ParserV2:
    ...
    def parse_lis(self) -> dict:
        ...
        if lis_version != "v2":
            logger.warning(
                "File does not appear to be v2 format. Attempting to parse anyway."
            )

        return self._parse_v2(schema_version or "unknown")
