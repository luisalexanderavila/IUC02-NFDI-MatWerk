"""
translate_bam_data.py - Convert a BAM LIS file to a JSON conforming to
the 2026-06 data schema (v2.1.8).

Usage:
    python translate_bam_data.py <path/to/file.lis> [-o output.json]

The script:
1. Parses the LIS file using LisParse.ParserV2.
2. Applies the mapping document (BAM2schema.json) to produce a JSON that
    mirrors the 2026-06_Data-Schema_Creep_v2.1.8.json structure.
...
_dep_path = os.path.join(_deps_dir, "LISParser")
...
_lisparse_file = os.path.join(_lisparser_module_dir, "LisParse.py")
_spec = importlib.util.spec_from_file_location("LisParse", _lisparse_file)
if _spec is None or _spec.loader is None:
    raise ImportError(f"Unable to load LisParse from {_lisparse_file}")
...
DEFAULT_MAPPING_FILE = os.path.join(_parsing_root, "Metadata", "Mappings", "BAM2schema.json")
...
def _build_arg_parser():
    parser = argparse.ArgumentParser(
        ...
        description="Translate a BAM LIS file to schema 2026-06 JSON",
    )
    ...
    parser.add_argument(
        "--mapping", "-m",
        dest="mapping",
        help="Mapping document. Defaults to Metadata/Mappings/BAM2schema.json.",
        type=str,
        default=None,
    )
    parser.add_argument(
...
def main(argv=None):
    ...
    if lis_version == "v1":
        Logger.warning(
            ...
            "Only v2 (2026) LIS format is supported."
        )
